...
| 8 | 数据级时序图（L1 波形） | **逻辑分析仪式逐位方波**（ScottPlot.WPF，独立图表窗口 ChartWindow，主窗顶部「波形」按钮打开——独立顶层窗口不设 Owner，2026-09-26 起；2026-09-02 由接收区下方内嵌迁入）：按字节+波特率重建 UART 位流（起始位0+8数据位LSB在前+停止位1），RX/TX 双通道电平带并行显示；跳变点展开成方波拐点渲染；跟随窗口 100ms（位级观察）/自由缩放平移/双击复原；跳变缓冲 6 万点上限；开关与跟随状态持久化。注：按字节重建的合成波形（串口无电平采样能力，真实电平需逻辑分析仪硬件，见 L3） | V1.2 ✅ |
...
- **最小窗口宽 1290（2026-09-19 由 1220 上调）**：顶部功能条加入「终端」复选框后实测不换行阈值 ~1246（原 2026-09-01 阈值 ~1185 不含「终端」），+ 字体度量余量取 1290；2026-09-26「波形/终端」复选框改按钮（按钮 MinWidth 64，两枚共加宽 ~38 → 阈值 ~1284，仍在 1290 内）；窄于该值这些行折行，MinWidth 与默认宽一致
...
- **图表面板独立窗口（2026-09-02）**：时序图 / 字段曲线整体迁出主窗为 `ChartWindow`，**不设 Owner、与主窗平级的独立顶层窗口**（2026-09-26：曾设 Owner=主窗，从属窗口被绑进主窗激活组——开窗/激活会强制带出（含最小化中的）主窗、永远浮于主窗之上、随主窗最小化还原联动，用户报「新窗强制拉起主窗」后移除）；主窗顶部「波形」**按钮**打开（已开则带到前台），**默认关闭**（2026-09-03：启动不自动弹图表窗，用户按需打开，打开状态仍记忆——上次退出时开着则随启动打开），点图表窗 X ⇔ 回写 ShowWavePanel=false 销毁；销毁/重建代替隐藏（2026-09-03 终版，Hide/重显不可靠，见下条）；初始位置贴主窗右侧、之后记住用户拖出的位置。主窗 Closing 时强制真关图表窗，否则其「X=回写关闭」语义会取消关闭导致进程不退。终端窗 `TerminalWindow` 与本窗同一套生命周期（2026-09-26 同轮去 Owner + 入口改按钮）
...
- **图表窗销毁/重建 + 补帧（2026-09-03，终版）**：图表窗关闭（X 回写关闭；2026-09-26 起主窗入口为按钮，无勾选态）即销毁、重开新建（记忆位置尺寸）——不使用 Hide/重显（补帧不可靠）；空闲（无收发数据）时渲染事件不触发且 ScottPlot 首帧不主动重绘，`OnContentRendered/SizeChanged/Activated/SelectionChanged` 延迟补帧 + 1Hz 保底。**坐标轴消失事故的真正根因**：帮助弹层 Popup 曾放在字段曲线页 DockPanel 内、画布 Border 之后——DockPanel 最后一个子元素接管填充职责，0 尺寸的 Popup 把默认 Dock=Left 的画布挤成 0 宽（时序图页无弹层故正常）；Popup 已移至窗口根 Grid，画布 Border 保持 DockPanel 末位
...
- **等宽输入 28px 基线（2026-09-01）**：`Mono` 样式只换字体族（Cascadia 行高偏大，靠收紧行距落回基线），表格/工具条中 Mono 框与普通框同高、字号全局统一（2026-09-26 起字体族改引 `MonoFont` 动态资源——默认仍是 Cascadia 链，用户在外观设置选字体后跟随替换）
...
- **界面字体自定义（2026-09-26）**：外观设置窗新增「界面字体」行——字体族下拉（系统字体按名排序 + 首项空串哨兵「（跟随系统）」，**字体名统一用系统消息字体渲染**（`SystemFonts.MessageFontFamily` 钉死整个 ComboBox 含弹层），`VirtualizingStackPanel` 显式虚拟化防数百项展开卡顿）+ 字号下拉（「默认」+ 9~20 预置），`SelectedValue` 双向绑 `Appearance.UiFontFamily/UiFontSize`（string 存储，与 HEX 同模式），选择即时级联 + 经既有「单例 PropertyChanged → 落盘」链自动保存；行尾不放说明文字——窗口内容 StackPanel 定宽 484，横向行超宽即被窗口右缘裁切（首版行尾说明文字实测裁切，说明收进 ToolTip）。**踩坑（字体列表乱码）**：首版每项用该字体自身渲染名字（所见即所得），实测符号字体（Bookshelf Symbol 7 等）的字母被映射成符号图形——名字显示为乱码，且用户若选中符号字体整个界面变乱码；改统一消息字体渲染后任何字体名可读，用户误选符号字体时列表仍可读、能选回「（跟随系统）」恢复。**覆盖范围 = 全部界面文字**：① 9 个窗口（主/终端/图表/外观/模板编辑 + 4 对话框）根元素挂 `FontFamily/FontSize={DynamicResource UiFontFamily/UiFontSize}` 继承入口（未显式设字体的子元素全跟随；标题 14/说明 12/指纹 11 等层级微调保留不跟随字号）；② 等宽区 `Mono`/`MonoRich` 样式 FontFamily 改引 `MonoFont` 资源（App.xaml 预定义 Cascadia 默认链，用户选字体后 `App.ApplyUiFont` 同步替换，等宽区跟随用户字体）；③ 自绘终端 `TerminalView.SetFont`（重建 4 个 Typeface + `_metricsReady=false` 度量重算 + `_ftCache` 整清 + IME 隐藏框字体/尺寸跟随 + `InvalidateVisual`；格宽行高变化由 OnRender 反推 cols/rows 自动 `Resize` reflow 并发 `Resized` 联动 PTY 窗口变更），构造时经 `ResolveFont()` 对齐当前设置、运行时由 `TerminalWindow.OnAppearanceChanged` 广播全部标签。**「跟随系统」的实现手段**：`UiFontFamily/UiFontSize` 两资源在 App.xaml **不预定义**——DynamicResource 无 key 可解析时属性不被设置 = 窗口继承 WPF 系统消息字体；用户清空选择时 `Resources.Remove` 回到该态（未设置零视觉变化，老配置升级无感）。校验：字体名须 `Fonts.SystemFontFamilies` 名单内（OrdinalIgnoreCase）、字号 6~72，坏值按默认启动
...
| V1.4.x | 终端/图表窗口独立化 + 主窗入口改按钮：`ChartWindow`/`TerminalWindow` 去 `Owner`（从属窗口被绑进主窗激活组 = 开窗强制拉起主窗的根因），改与主窗平级的独立顶层窗口；主窗「波形」「终端」复选框改按钮（点按开窗/已开置前，关闭只走窗口 X），显隐统一由 `ShowWavePanel`/`ShowTerminalPanel` 属性变化驱动（复选框 Checked/Unchecked 事件链移除） | `MainWindow.xaml(.cs)`（按钮 + OnVmPropertyChanged 驱动 + 去 Owner）、`ChartWindow`/`TerminalWindow`（注释语义） | ✅ 完成（2026-09-26：构建零错误、107/107 单测） |
...
| V1.4.x | 界面字体自定义（全局字体族 + 字号）：外观设置「界面字体」行（系统字体下拉——字体名统一系统消息字体渲染（符号字体自渲染乱码教训弃所见即所得）+ 虚拟化防卡 + 空串哨兵「（跟随系统）」、字号「默认」+9~20，`SelectedValue` 双向绑单例即时生效落盘；行尾说明移除防定宽窗裁切）；9 窗口根元素 DynamicResource 继承入口 + `Mono`/`MonoRich` 改引 `MonoFont` 资源（用户字体覆盖等宽区、空则回退 Cascadia 默认链）+ 终端 `SetFont`（Typeface 重建/度量失效/缓存整清/IME 框跟随/自动 reflow）经 `TerminalWindow` 广播；**「跟随系统」= 两资源不预定义 + DynamicResource 未解析不应用**（未设置零视觉变化）；字体名系统名单校验 + 字号 6~72 坏值回退 | `Services/Appearance.cs`（UiFontFamily/UiFontSize）、`App.xaml(.cs)`（MonoFont 资源 + ApplyUiFont）、9 窗口 XAML 根元素、`Controls/TerminalView.cs`（SetFont/ResolveFont）、`TerminalWindow.xaml.cs`（广播）、`AppearanceWindow.xaml(.cs)`（字体行）、`MainViewModel.cs`（UiSettings 两字段 + 校验） | ✅ 完成（2026-09-26：构建零错误；截图实测微软雅黑 16 全局生效/外观窗选择即时应用/终端框架+内容区（本地 PowerShell）换字体/重启持久化/恢复默认回退/坏值防护；二轮修复行尾说明裁切与符号字体列表乱码） |
