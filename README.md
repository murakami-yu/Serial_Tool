...
| V1.4 | 终端/Shell：XTerm.NET 终端仿真（WPF 自绘，零 WebView）—— M1 串口/TCP VT 终端模式 ✅ → M2 SSH.NET 远程会话（host key TOFU）→ M3 多会话标签（[执行计划](Docs/Shell功能执行计划.md)） | 🚧 进行中（2026-09-18：M0/M1 完成，构建+92 单测+冒烟通过；回环/真机验收待人工） |
