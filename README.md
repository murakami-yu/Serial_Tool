...
| V1.4.x | 界面字体自定义（全局字体族 + 字号）：外观设置新增「界面字体」行——系统字体下拉（**字体名统一用系统消息字体渲染**，弃「逐项自身字体渲染所见即所得」方案——符号字体如 Bookshelf Symbol 7 的字母被映射成符号图形、名字成乱码；整框钉死 MessageFontFamily 保证用户已选符号字体时列表仍可读能选回；虚拟化面板防数百项卡顿）+ 字号下拉（默认/9~20）；选择即时生效并落盘。**覆盖全部界面文字**：9 个窗口根元素 `FontFamily/FontSize={DynamicResource}` 继承入口 + 等宽区（Mono/MonoRich 样式改引 `MonoFont` 资源，用户选字体后跟随、未选时保持 Cascadia 默认链）+ 自绘终端（`SetFont`：重建 Typeface/度量失效/文字缓存整清/IME 框跟随，格宽行高重算自动 reflow + PTY resize 联动，`TerminalWindow` 广播全部标签）；未设置时维持现状零视觉变化（全局资源不存在 = DynamicResource 不应用 = 继承系统默认，这是「跟随系统」的实现手段）；字体名须系统已装名单内、字号 6~72，坏值回退默认 | ✅ 完成（2026-09-26，截图实测微软雅黑 16 全局生效/外观窗下拉恢复与选择/终端框架+内容区（本地 PowerShell）即时换字体/重启持久化/恢复默认回退/坏值防护；二轮修复行尾说明裁切与符号字体列表乱码） |
